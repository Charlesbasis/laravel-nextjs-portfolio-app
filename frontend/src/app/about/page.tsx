...
import { useCertifications, useCurrentProfile, useEducation, useExperiences, useUserStats } from '@/src/hooks/useProfile';
import {
  AlertCircle,
  Award, Briefcase,
  Clock,
...
export default function About() {
  ...
  // Destructured only 'data' to remove unused loading warnings
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Profile Not Set Up</h2>
          <p className="text-gray-600 mb-6">
            You haven&apos;t set up your profile yet. Please complete your profile to view this page.
          </p>
          <Link href="/dashboard/settings" className="inline-flex items-center bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-colors">
            Set Up Profile
          </Link>
        </div>
      </div>
                ...
                <div className="space-y-4">
                  {certifications.map((cert) => (
                    <div key={cert.id} className="bg-gray-50 rounded-xl p-6 hover:shadow-lg transition-shadow">
                      ...
                      <p className="text-gray-600 text-sm">{new Date(cert.issue_date).toLocaleDateString()}</p>
                    </div>
                  ))}
                </div>
              </div>
