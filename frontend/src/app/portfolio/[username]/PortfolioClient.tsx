...
export default function PortfolioClient({ username, initialData }: { username: string, initialData?: Record<string, unknown> }) {
  const { data: profile, isLoading: profileLoading, isError: profileError } = usePublicProfile(username, initialData);
  const { data: stats } = useUserStats(username);
  const { data: experiences } = usePublicExperiences(username);
  const { data: education } = usePublicEducation(username);
  const { data: certifications } = usePublicCertifications(username);
  ...
  return (
    <>
      ...
      <StructuredData profile={profile} projects={projects} experiences={experiences || []} />
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <ProjectsGrid projects={projects} isLoading={projectsLoading} />
        <SkillsDisplay skills={skills} isLoading={skillsLoading} />
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="text-center max-w-md">
        ...
        <Link href="/" className="bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold">
          Go to Homepage
        </Link>
      </div>
    </div>
